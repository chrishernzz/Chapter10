...
void challenge1() {
	...
	//now set them to branch1 (branch1 is our ancestor (parent) basically with children which are the leaf1 and leaf2)
	branch1.setLeft(&leaf1);
	branch1.setRight(&leaf2);
	cout << "\n\n";
	print_tree("\t\t", &tree, tree.isLeaf(), true, true);
	cout << "\n\t\tRight branch of (branch #2) grows two leaves: leaf #3 and leaf #4.";
	//now same thing but now branch2 will have another subtree with 2 leaves which are the children 
	Binary_Tree_Node<string> leaf3, leaf4;
	leaf3.setData("leaf #3");
	leaf4.setData("leaf #4");
	//now set left and right to branch2, now branch2 who is the parent will have children that are leaf3 and leaf4
	...
	//now going to the parent leaf#3 to have children which are orange and coconut (they are siblings now) 
